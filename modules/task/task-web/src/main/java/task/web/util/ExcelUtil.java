...
public class ExcelUtil {
	...
	public static void exportTemplate(ResourceRequest resourceRequest, ResourceResponse resourceResponse, String templateName, List<String> headers) {
		try {
			//Excel header must be in the default language of the system
			XSSFWorkbook excel = ExcelUtil.excelHeader(resourceRequest, headers);
			// Transform Excel into ByteArrayOutputStream
			...
			ServletResponseUtil.sendFile(PortalUtil.getHttpServletRequest(resourceRequest),
					PortalUtil.getHttpServletResponse(resourceResponse), templateName, baos.toByteArray(), "application/download");
		} catch (Exception e) {
			_log.error(e,e);
		}
	}
	...
	public static List<String> getColumnNamesMap(PortletRequest portletRequest, List<String> headers) {
		...
		for (String header : headers) {
			list.add(LanguageUtil.get(ResourceBundle.getBundle(ExcelUtilConstants.CONTENT_LANGUAGE, portletRequest.getLocale()), header));
		}
		return list;
	}
	...
	public static XSSFWorkbook excelHeader(ResourceRequest actionRequest, List<String> headers) {
		...
		for (int column = 0; column < headers.size(); column++) {
			...
			createStringCell(row, cellStyle, column,
					LanguageUtil.get(ResourceBundle.getBundle(ExcelUtilConstants.CONTENT_LANGUAGE, actionRequest.getLocale()), headers.get(column)));
		}
		

		// Setting the cell type to "text" for an entire column
	...
	public static List<String> getExcelHeaders(Sheet sheet) {
		...
		Row row = sheet.getRow(0); // Getting header Row
		if (row != null) {
			Cell cell;
	...
	public static boolean validateExcelHeaders(ActionRequest actionRequest, List<String> excelHeaders,
		...
		if (excelHeaders.size() != columnNames.size()) {
			msgs.add(LanguageUtil.get(ResourceBundle.getBundle(ExcelUtilConstants.CONTENT_LANGUAGE, actionRequest.getLocale()), "self.registration.web.admin.import-template-not-up-to-date"));
			isValid = false;
		} else {
			for (int i = 0; i < excelHeaders.size(); i++) {
				String header = excelHeaders.get(i);
				if (!excelHeaders.get(i).equals(structureHeaders.get(i)) || (!structureHeaders.contains(header))) {
					msgs.add(LanguageUtil.format(ResourceBundle.getBundle(ExcelUtilConstants.CONTENT_LANGUAGE, actionRequest.getLocale()), invalidHeader,
							new String[] {header, String.valueOf(i+1), structureHeaders.get(i)}));
					isValid = false;
				} else if (excelHeaders.lastIndexOf(header) > i) {
					msgs.add(LanguageUtil.format(ResourceBundle.getBundle(ExcelUtilConstants.CONTENT_LANGUAGE, actionRequest.getLocale()), repeatedHeader,
							new String[]{ header, String.valueOf(i+1), String.valueOf(excelHeaders.lastIndexOf(header)+1)}));
					isValid = false;
				} else if (header.length() == 0) {
					msgs.add(LanguageUtil.format(actionRequest.getLocale(), invalidHeader,
	...
	public static XSSFWorkbook exportexcel(ResourceRequest request, List<String> headers, Map<String, Object[]> content) {
		// Create excel template
		XSSFWorkbook excel = new XSSFWorkbook();
		XSSFSheet excelSheet = excel.createSheet();
		// Sheet name
		...
		for (int column = 0; column < headers.size(); column++) {
			excelSheet.setColumnWidth(column, 5000);
			createStringCell(row, cellStyle, column,
					LanguageUtil.get(ResourceBundle.getBundle(ExcelUtilConstants.CONTENT_LANGUAGE, request.getLocale()), headers.get(column)));
		}
	...
	public static JSONObject getExcelValues(List<String> fieldNames, List<String> excelRowValues) {
		...
		for (int i = 0; i < excelRowValues.size(); i++) {
			String value = excelRowValues.get(i);
